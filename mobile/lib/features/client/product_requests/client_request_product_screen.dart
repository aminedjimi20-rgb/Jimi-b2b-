...
/// Client can't find a product in the catalog — snap/pick a photo of what
/// they want and send it to the Admin, who reviews it from
/// AdminProductRequestsScreen and closes it out with a status + a note.
class ClientRequestProductScreen extends ConsumerStatefulWidget {
  const ClientRequestProductScreen({super.key, this.initialImagePath});
  final String? initialImagePath;

  @override
  ConsumerState<ClientRequestProductScreen> createState() => _ClientRequestProductScreenState();
}

class _ClientRequestProductScreenState extends ConsumerState<ClientRequestProductScreen> {
  String? _pickedPath;
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(productRequestsApiProvider).create(
            imageUrl: imageUrl,
  ...
  Widget build(BuildContext context) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Votre demande a été envoyée. Vous serez notifié dès qu\'elle sera traitée.', textAlign: TextAlign.center),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text(
            'Vous ne trouvez pas un produit dans le catalogue ? Envoyez-en une photo, l\'équipe vous répondra dès que possible.',
          ),
          const SizedBox(height: 16),
          Row(
            children: [
